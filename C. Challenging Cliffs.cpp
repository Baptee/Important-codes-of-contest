    ...
    test{
        lli n;cin>>n;
   int a[n];for(lli i=0;i<n;i++){cin>>a[i];v.push_back(a[i]);}
       sort(v.begin(), v.end());
         lli mini=2e9;
         lli ind=-1;
            for(int i=1;i<n;i++){
    ...
    for(int i=0;i<n;i++){
        ...
        if(v[i]>v[ind-1]) big.push_back(v[i]);
    }
    for(int i=0;i<n;i++){
        if(i==ind || i==ind-1) continue;
        if(v[i]<=v[ind]) small.push_back(v[i]);
    }
    cout<<v[ind-1]<<" ";
     for(lli i=0;i<big.size();i++) cout<<big[i]<<" ";
     for(lli i=0;i<small.size();i++)cout<<small[i]<<" ";
    cout<<v[ind]<<" "<<"\n";
